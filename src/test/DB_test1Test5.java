...
class DB_test1Test5 {
	...
	void test() {
		...
		DBupdate("DELETE from Routes where USERNAME='"+USERNAME+"'");
		ResultSet RS=null;
		RS=DBqueryRS("select count(*) from WebUsers.USERNAME where USERNAME='"+USERNAME+"'", RS);
		
		assertEquals((int)DBqueryINT("select count(*) from WebUsers.USERNAME where USERNAME='"+USERNAME+"'"),0);
